...
void ofCvContourFinder::draw( float x, float y ) {
...
void ofCvContourFinder::findContours( ofxCvGrayscaleImage&  input,
									  int minArea,
									  int maxArea,
									  int nConsidered,
									  bool bFindHoles) {
	...
	for( int i = 0; i < MIN(nConsidered, nCvSeqsFound); i++ ) {
		...
		for( int j=0; j < MIN(MAX_CONTOUR_LENGTH, cvSeqBlobs[i]->total); j++ ) {
